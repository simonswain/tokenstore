...
module.exports = function(opts){
  ...
  api.makeHash = function(val, ttl, done){
    if(typeof(ttl) === 'function'){
      done = ttl;
      ttl = 3600;
    }
    var hash = crypto.randomBytes(8).toString('hex');
    var key = opts.prefix + ':hash:' + hash;
    redis.setex(key, 3600, String(val), function(){
      done(null, hash);
    });
  };

  api.getHash = function(hash, next){
    var key = opts.prefix + ':hash:' + hash;
  ...
  api.delHash = function(hash, next) {
    var key = opts.prefix + ':hash:' + hash;
  ...
  api.reset = function(done) {
    ...
    var clearHashes = function(next){
      // get all tokens and delete
      var key = opts.prefix + ':hash:*';
      ...
      redis.keys(key, function(err, list){
        ...
        var delHash = function(hash, cb){
          redis.del(hash, function(err) {
            return cb();
          });
        };

        async.eachSeries(list, delHash, next);
      });
